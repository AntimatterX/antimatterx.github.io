/*!
 * CmdExt JavaScript Library v2.0.10
 ...
 * CmdExt
 * 
 * Last Update: 2021-04-03T17:18:38.462Z
 */
(function _main(_root, undefined) {
    ...
    var _lib = {
        ...
        val: (function () { // エクスポートされるライブラリの値
             ...
             * @param {Object<string, string|null|AsyncFunction|Function|Array<string>|Object<string, ?*>|boolean>} options オプションを渡します。
            ...
            function CmdExt(options) {
                ...
                options = CmdExt._castParam(options, {
                    ...
                    converter: ['AsyncFunction', 'Function', 'Boolean'],
                    quots: 'String',
                    bridge: /^.*$/
                });
            ...
            CmdExt.parseArg = function (str, quots) {
                ...
                if (quotsPattern === '(?:()(?!\\\\))' ||
                    ...
                    var argRegExp = new RegExp('(?:^|\\s+)' + quotsPattern + '[\\s\\S]*?\\1(?:$|\\s+)|\\S+');
                    while (argRegExp.test(reversed)) {
                        args.push(reversed.match(argRegExp)[0].trim());
                        reversed = reversed.replace(argRegExp, '');
                    }
                }
             ...
             */
            CmdExt.version = CmdExt.prototype.version = '2.0.10';
            /**
            ...
            CmdExt.prototype.exec = function (str) {
                ...
                if (ctx.command === undefined || !ctx.command.enabled) {
                    if ((ctx.command === undefined && this._handlers.commandnotfound.length > 0) ||
                        (ctx.command !== undefined && !ctx.command.enabled && this._handlers.disabledcommand.length > 0)) this.trigger(
                            ctx.command === undefined ? 'commandnotfound' : 'disabledcommand', ctx
                        );
                    else if (this._handlers.commanderror.length === 0) throw ctx.error;
            ...
            /**
             * ハンドラーを登録します。
             * @param {string|Object<string, AsyncFunction|Function>} type 空白区切りのイベント名の文字列かイベント名とハンドラーが組のオブジェクトを渡します。
             * @param {AsyncFunction|Function} [handler=undefined] ハンドラーの関数を渡します。
             * @returns {CmdExt} インスタンス自身を返します。
             */
